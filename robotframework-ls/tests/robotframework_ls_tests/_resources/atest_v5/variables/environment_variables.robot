...
#!                   ^^^^^^^ Undefined environment variable: TEMPDIR
...
Non-Existing Environment Variable
    ...
    Log  %{NON_EXISTING}
#!         ^^^^^^^^^^^^ Undefined environment variable: NON_EXISTING
...
#!         ^^^^^^^^^^^^^^^^^^^ Undefined environment variable: THIS ENV VAR IS SET

Environment Variables Are Space Sensitive 2
    [Documentation]  FAIL Environment variable '%{ THIS_ENV_VAR_IS_SET }' not found. Did you mean:
    ...    ${SPACE * 4}\%{THIS_ENV_VAR_IS_SET}
    Log  %{ THIS_ENV_VAR_IS_SET }
#!         ^^^^^^^^^^^^^^^^^^^^^ Undefined environment variable:  THIS_ENV_VAR_IS_SET 
...
#!         ^^^^^^^^^^^^^^^^^^^ Undefined environment variable: TH_IS_ENVVAR_IS_SET
