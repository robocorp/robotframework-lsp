...
def test_gherkin_at_beginning_of_keyword_should_be_highlighed(workspace):
    robot_source_file = set_test_case_with_keyword(
        "Given Some BDD description of desired system behaviour", workspace
    )
    ...
    assert not semantic_tokens.get(
        "Given Some BDD description of desired system behaviour"
    )


def test_gherkin_when_should_be_highlighted(workspace):
    robot_source_file = set_test_case_with_keyword(
        "When Some BDD description of desired system behaviour", workspace
    )
    semantic_tokens = get_semantic_tokens_from_language_server(
        workspace, robot_source_file
    )
...
def test_gherkin_then_should_be_highlighted(workspace):
    robot_source_file = set_test_case_with_keyword(
        "Then Some BDD description of desired system behaviour", workspace
    )
    semantic_tokens = get_semantic_tokens_from_language_server(
        workspace, robot_source_file
    )
...
def test_gherkin_and_should_be_highlighted(workspace):
    robot_source_file = set_test_case_with_keyword(
        "And Some BDD description of desired system behaviour", workspace
    )
    semantic_tokens = get_semantic_tokens_from_language_server(
        workspace, robot_source_file
    )
...
def test_gherkin_but_should_be_highlighted(workspace):
    robot_source_file = set_test_case_with_keyword(
        "But Some BDD description of desired system behaviour", workspace
    )
    semantic_tokens = get_semantic_tokens_from_language_server(
        workspace, robot_source_file
    )
...
def test_false_gherkin_in_middle_of_keyword_should_not_be_highlighted(workspace):
    robot_source_file = set_test_case_with_keyword(
        "Then The system under test has given something", workspace
    )
    semantic_tokens = get_semantic_tokens_from_language_server(
        workspace, robot_source_file
    )
    assert (
        semantic_tokens.get("The system under test has given something")
        == "keywordNameCall"
    )
...
def test_library_prefix_should_be_highlighted(workspace):
    robot_source_file = set_keyword_in_keyword_section(
        "Requests.GET    url=https://github.com/robocorp/robotframework-lsp/issues/581",
        workspace,
    )
    semantic_tokens = get_semantic_tokens_from_language_server(
        workspace, robot_source_file
...
def test_module_prefix_abuse_in_bdd_should_be_supported(workspace):
    robot_source_file = set_test_case_with_keyword(
        "Given Module.Keyword    With parameter", workspace
    )
    semantic_tokens = get_semantic_tokens_from_language_server(
        workspace, robot_source_file
    )
...
def test_keywords_should_not_be_mistaken_for_gherkin(workspace):
    robot_source_file = set_test_case_with_keyword(
        "Buttercup    ${argument_1}    ${argument_2}", workspace
    )
    semantic_tokens = get_semantic_tokens_from_language_server(
        workspace, robot_source_file
    )
