...
**b.** Use the `robot.libraries.libdoc.needsArgs` to specify that the arguments typed in the editor should be passed to the library to initialize it.
    ...
    *** Settings ***
    ...
    Note that in such circumstances, if the library is used from different places with different arguments
    each place will generate a different libspec (as different arguments could result in different keywords.

**c.** Manually generate the libspec for the library and put it somewhere in your workspace or right below a folder in
the `PYTHONPATH`.
