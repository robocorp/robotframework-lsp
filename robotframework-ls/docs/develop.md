...
- Update version (`python -m dev set-version 0.4.3`).
...
- Create a tag (`git tag robotframework-lsp-0.4.3`) and push it.

- Send release msg. i.e.:

`Robot Framework Language Server 0.4.3` is now available (both from `pip install robotframework-lsp` or through the `VSCode marketplace`).
