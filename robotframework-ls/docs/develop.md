...
- Update version (`python -m dev set-version 0.9.1`).
...
- Update build.gradle patchPluginXml.changeNotes with the latest changelog (html expected).
...
- Create a tag (`git tag robotframework-lsp-0.9.1`) and push it.

- Send release msg. i.e.:

`Robot Framework Language Server 0.9.1` is now available (both from `pip install robotframework-lsp` or through the `VSCode marketplace`).
